...
JSVMRuntime::JSVMRuntime(
  ...
  OH_JSVM_SetInstanceData(
      env,
      reinterpret_cast<facebook::react::MessageQueueThread*>(jsQueue.get()),
      nullptr,
      nullptr);
}
...
Value JSVMRuntime::evaluateJavaScript(
  ...
  CALL_JSVM_AND_THROW(OH_JSVM_CompileScript(
      env,
      *jsSrc,
      cache.empty() ? nullptr : cache.data(),
      cache.size(),
      true,
      &cacheRejected,
      script.get()));
...
std::shared_ptr<const PreparedJavaScript> JSVMRuntime::prepareJavaScript(
  ...
  return std::make_shared<const JSVMPreparedJavaScript>(
      dataPtr, lengthPtr, buffer);
}

Value JSVMRuntime::evaluatePreparedJavaScript(
    const std::shared_ptr<const PreparedJavaScript>& js) {
  DFX();
  JSVMUtil::HandleScopeWrapper scope(env);

  // codeCache还原为JSVM_Script
  auto sourceJs = std::static_pointer_cast<const JSVMPreparedJavaScript>(js);
  JSVM_Value jsSrc;
  CALL_JSVM_AND_THROW(OH_JSVM_CreateStringUtf8(
      env,
      reinterpret_cast<const char*>(sourceJs->source()),
      sourceJs->sourceLength(),
      &jsSrc));
  bool cacheRejected = true;
  JSVM_Script script;
  CALL_JSVM_AND_THROW(OH_JSVM_CompileScript(
...
PropNameID JSVMRuntime::createPropNameIDFromAscii(
    const char* str,
    size_t length) {
  DFX();
  JSVMUtil::HandleScopeWrapper scope(env);
...
PropNameID JSVMRuntime::createPropNameIDFromUtf8(
    const uint8_t* utf8,
    size_t length) {
  DFX();
  JSVMUtil::HandleScopeWrapper scope(env);
  ...
  JSVM_Value value = nullptr;
  OH_JSVM_CreateStringUtf8(
      env, reinterpret_cast<const char*>(utf8), length, &value);
  return JSVMConverter::make<PropNameID>(env, value);
}
...
bool JSVMRuntime::compare(
    const PropNameID& propNameID1,
    const PropNameID& propNameID2) {
  DFX();
  JSVMUtil::HandleScopeWrapper scope(env);
...
bool JSVMRuntime::bigintIsInt64(const BigInt& value) {
  ...
  const JSVM_Value jsvm_value = JSVMConverter::PointerValueToJSVM(env, value);
...
String JSVMRuntime::bigintToString(const BigInt& value, int radix) {
  ...
  CALL_JSVM(
      env,
      OH_JSVM_CreateStringUtf8(
          env, bigIntStringStr, strlen(bigIntStringStr), &bigIntString));
  ...
  CALL_JSVM(
      ...
      OH_JSVM_CallFunction(
          env, global, bigintClass, 1, bigInt_args, &bigintNewResult));
  ...
  CALL_JSVM(
      env,
      OH_JSVM_CreateStringUtf8(
          env, toStringStringStr, strlen(toStringStringStr), &toStringString));
  CALL_JSVM(
      env,
      OH_JSVM_GetProperty(
          env, bigintNewResult, toStringString, &bigintToStringFunction));
  ...
  JSVM_Status status = OH_JSVM_CallFunction(
      env,
      bigintNewResult,
...
Object JSVMRuntime::createObject(std::shared_ptr<HostObject> ho) {
  ...
  if (unlikely(!hostObjectClass)) {
    ...
    CALL_JSVM_AND_THROW(OH_JSVM_DefineClassWithPropertyHandler(
        env,
        className,
        len,
        ctor,
  ...
  CALL_JSVM_AND_THROW(OH_JSVM_NewInstance(
      env, localHostObjectClass, 0, nullptr, &instanceValue));
...
std::shared_ptr<HostObject> JSVMRuntime::getHostObject(const Object& object) {
  DFX();
  JSVMUtil::HandleScopeWrapper scope(env);
  JSVM_Value jsObject = JSVMConverter::PointerValueToJSVM(env, object);
...
bool JSVMRuntime::hasProperty(const Object& value, const PropNameID& name) {
  ...
  const JSVM_Value jsvm_object = JSVMConverter::PointerValueToJSVM(env, value);
...
bool JSVMRuntime::isArray(const Object& value) const {
  ...
  bool result = false;
  JSVM_Status status = OH_JSVM_IsArray(env, jsvm_value, &result);
...
bool JSVMRuntime::isArrayBuffer(const Object& value) const {
  ...
  bool result = false;
  JSVM_Status status = OH_JSVM_IsArraybuffer(env, jsvm_value, &result);
...
bool JSVMRuntime::isFunction(const Object& value) const {
  ...
  bool result = false;
  JSVM_Status status = OH_JSVM_IsFunction(env, jsvm_value, &result);
...
ArrayBuffer JSVMRuntime::createArrayBuffer(
  ...
  CALL_JSVM(
      env,
      OH_JSVM_CreateArraybuffer(env, buffer->size(), &arrayBufferPtr, &ptr));
...
size_t JSVMRuntime::size(const ArrayBuffer& arrayBuffer) {
  ...
  CALL_JSVM(
      env, OH_JSVM_GetArraybufferInfo(env, value, nullptr, &arrayBufferLength));
...
void JSVMRuntime::setValueAtIndexImpl(
    const Array& array,
    size_t i,
    const Value& value) {
  DFX();
...
Value JSVMRuntime::call(
  ...
  JSVM_Value result = nullptr;
  CALL_JSVM_AND_THROW(OH_JSVM_CallFunction(
      env, receiver, funcValue, count, argsValue.data(), &result));
  return JSVMConverter::JSVMToJsi(env, result);
}

Value JSVMRuntime::callAsConstructor(
    const Function& func,
    const Value* args,
    size_t count) {
  DFX();
  ...
  CALL_JSVM(
      env,
      OH_JSVM_NewInstance(
          env, funcValue, count, argsValue.data(), &resultValue));
...
void JSVMRuntime::setExternalMemoryPressure(const Object& obj, size_t amount) {}
...
void JSVMRuntime::UpdateCodeCache(
    const std::string& sourceURL,
...
void JSVMRuntime::UpdateCodeCacheL2(
    const std::string& sourceURL,
    const std::vector<uint8_t>& buffer) {
  std::lock_guard<std::mutex> lock(codeCacheMtx);
  DLOG(INFO) << "Update L2 CACHE: " << sourceURL
             << "; size = " << buffer.size();
