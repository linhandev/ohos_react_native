...
class JSBigStdVecString final : public JSBigString {
 public:
  JSBigStdVecString(std::vector<uint8_t> buffer, bool isAscii = false)
...
std::unique_ptr<JSBigString const> fromRawFilePath(
  ...
  std::unique_ptr<RawFile, decltype(&OH_ResourceManager_CloseRawFile)>
      bundleRawFile(
          OH_ResourceManager_OpenRawFile(
              nativeResourceManager, rawFilePath.c_str()),
  ...
  if (!success) {
    throw std::runtime_error(
        "Failed to get raw file descriptor data: " + rawFilePath);
  }

  auto result = std::make_unique<JSBigFileString>(
      bundleRawFileDescriptor.fd,
