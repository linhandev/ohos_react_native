...
std::deque<HostObjectProxy::GetterContext> HostObjectProxy::getterStack;
...
JSVM_Value HostObjectProxy::Getter(
  ...
  JSVM_Value currentThis = thisArg;

  while (true) {
    JSVM_Status status = OH_JSVM_Unwrap(env, currentThis, &data);
    ...
    if (OH_JSVM_GetPrototype(env, currentThis, &prototype) != JSVM_OK) {
      OH_JSVM_ThrowError(
          env, nullptr, "HostObjectProxy.Getter: Failed to get prototype");
      return JSVMConverter::JsiToJSVM(env, Value::undefined());
    }

    bool isSameObject;
    if (OH_JSVM_StrictEquals(env, currentThis, prototype, &isSameObject) !=
        JSVM_OK) {
      OH_JSVM_ThrowError(
          env, nullptr, "HostObjectProxy.Getter: StrictEquals check failed");
      return JSVMConverter::JsiToJSVM(env, Value::undefined());
    }
    if (isSameObject) {
      OH_JSVM_ThrowError(
  ...
  try {
    ...
    std::string exceptionString(
        "Exception in HostObejctProxy.Getter: <unknown>");
    OH_JSVM_ThrowError(env, nullptr, exceptionString.c_str());
  }
...
JSVM_Value HostObjectProxy::Setter(
  ...
  try {
    ...
    std::string exceptionString(
        "Exception in HostObejctProxy.Setter: <unknown>");
    OH_JSVM_ThrowError(env, nullptr, exceptionString.c_str());
    return JSVMConverter::JsiToJSVM(env, Value::undefined());
  }
...
bool HostObjectProxy::isGetting(JSVM_Env env, JSVM_Value obj, JSVM_Value key) {
  for (const auto& context : getterStack) {
    if (context.env != env) {
      continue;
    }
    ...
    bool isSameObject;
    if (OH_JSVM_StrictEquals(env, context.obj, obj, &isSameObject) != JSVM_OK) {
      return false;
    }
    if (!isSameObject) {
      continue;
    ...
    bool isSameKey;
    if (OH_JSVM_StrictEquals(env, context.key, key, &isSameKey) != JSVM_OK) {
      return false;
    }
    if (!isSameKey) {
      continue;
