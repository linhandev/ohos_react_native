...
1. 打开 `AwesomeProject` 目录下的 `package.json`，在 `scripts` 下新增 HarmonyOS 的依赖：
   ```diff
   {
    "name": "AwesomeProject",
    "version": "0.0.1",
...
2. 在 `AwesomeProject` 目录下运行安装依赖包命令：
    ...
    ```
    > 指令中@x.x.x用于指定下载的版本，不指定时默认下载最新版本。
...
3. 将 `MyApplication/entry` 的 `oh_modules` 文件夹删除，点击 `entry` 文件夹，再点击顶部菜单栏的 `build`>`Clean Project` 清除项目缓存。
