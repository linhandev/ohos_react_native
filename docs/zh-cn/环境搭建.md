...
>
> 目前0.77分支的RNOH中会有两个release包，主要区别如下：
> - react_native_openharmony_release.har: 同0.72分支中的`react_native_openharmony_release.har`一样，这个包是在release模式下构建，cpp代码会经过编译转换成.so动态库，.ts和.ets文件保留源码形式；
...
1. 在 `MyApplication` 目录下新建 libs 文件夹，将 `react_native_openharmony_release.har` 或 `react_native_openharmony_release2.har` 放入该目录。
...
4. 将 `MyApplication/build-profile.json5` 中的 `useNormalizedOHMUrl` 设置成 `true`（如果您用的是 `react_native_openharmony_release.har` 可以忽略此步骤）：
...
5. 将 `MyApplication/entry` 的 `oh_modules` 文件夹删除，点击 `entry` 文件夹，再点击顶部菜单栏的 `build`>`Clean Project` 清除项目缓存。
6. 点击顶部菜单栏的 `File` > `Sync and Refresh Project` 来执行 `ohpm install`，执行完成后会在 `entry` 目录下生成 `oh_modules` 文件夹。
7. 点击顶部菜单栏的 `Run`>`Run 'entry'` 运行工程，运行成功后截图如下：
