...
2. 升级/添加RNOH前端依赖后，运行`npm i`安装依赖。若遇依赖冲突错误，可根据错误信息调整`package.json`中依赖库的版本来解决
...
3. 修改两处配置，其中第二处可选。注意只修改配置中下面列出的字段
    ...
    2. （可选）tsconfig.json
        ...
        ```
4. 打包前端bundle时，如遇到依赖冲突错误，可修改`package.json`、`metro.config.js`、`babel.config.js`等配置文件，根据错误提示进行调整即可
...
- [0.72.5到0.77.1版本上游社区不兼容变更梳理](./上游社区不兼容变更.md)
