...
以此法添加的trace会出现在对应的线程泳道下。以下是目前版本的各个线程：

|线程|泳道|详细信息|
|-|-|-|
|主线程|同应用包名|用于初始化、变更UI|
|JS线程|RNOH_JS|用于处理JS事件|
|BG线程|RNOH_BACKGROUND|不应出现此线程。如果出现，请关闭BG线程开关。|
|CLEANUP线程|RNOH_CLEANUP|用于释放资源|

可以通过在应用模块下的 `CMakeLists.txt` 中添加：

```CMAKE
add_compile_definitions(WITH_HITRACE_REACT_MARKER)
```

以启用REACT MARKER。以下是REACT MARKER封装的各个泳道：

|字段|类型|含义|
|-|-|-|
|RUN_JS_BUNDLE|时间段|运行JS Bundle|
|CREATE_REACT_CONTEXT|时间段|创建React上下文|
|REGISTER_JS_SEGMENT|时间段|注册JS段（Bundle）|
|CONTENT_APPEARED|时间点|内容出现|
|CHANGE_THREAD_PRIORITY|时间点|变更线程优先级|
|FABRIC_COMMIT|时间段|提交影子树|
|FABRIC_FINISH_TRANSACTION|时间段|挂载影子树|
|FABRIC_DIFF|时间段|差分影子树|
|FABRIC_LAYOUT|时间段|影子树布局|
|FABRIC_BATCH_EXECUTION|时间段|批量处理影子树变更指令|
|FABRIC_UPDATE_UI_MAIN_THREAD|时间段|Fabric更新UI属性|
|REACT_BRIDGE_LOADING|时间段|初始化JS引擎桥接|
|ON_HOST_RESUME|时间段|恢复计时器|
|ON_HOST_PAUSE|时间段|暂停计时器|
|REACT_INSTANCE_INIT|时间段|初始化RN实例|
|INITIALIZE_MODULE|时间段|初始化Turbo Module|
|PROCESS_PACKAGES|时间段|处理包|
|CREATE_UI_MANAGER_MODULE|时间段|创建UI管理器Turbo Module|
|CREATE_MODULE|时间段|创建Turbo Module|
|PROCESS_CORE_REACT_PACKAGE|时间段|处理核心包|
|DOWNLOAD|时间段|下载|
...
当发生崩溃后，通过以下方法可在 `DevEco Studio` 上查看到崩溃日志：

1. 手机连接到电脑，打开 `DevEco Studio`，打开一个项目；
2. 在 `DevEco Studio` 下方点击 `Log`；
3. 在 `FaultLog` 的筛选栏中选择你连接的设备、`[应用包名]`、`[崩溃类型]`、`[发生时间]`；

#### CppCrash

CppCrash是指发生在C++侧的崩溃。

更多CppCrash定界指导，请参考[CppCrash故障定位指导](https://developer.huawei.com/consumer/cn/doc/best-practices-V5/bpta-cppcrash-guidance-V5)

##### 关键信息

|字段|释义|详细信息|
|-|-|-|
|Device info|设备信息|通常为设备型号|
|Build info|构建信息|通常为系统版本|
|Module name|应用包名|app.json5中的bundleName字段|
|Version|应用版本|app.json5中的versionName字段|
|VersionCode|版本代码|app.json5中的versionCode字段|
|Foreground|是否前台|崩溃发生时的前后台状态|
|Timestamp|时间戳|崩溃发生时间|
|Pid|进程ID||
|Process name|进程名||
|Process life time|进程寿命||
|Reason|崩溃原因|详见下文|
|Fault thread info|报错线程信息|报错线程的调用栈和寄存器信息，详见下文|
|Other thread info|其他线程信息|其他线程的调用栈和寄存器信息，详见下文|
|HiLog|日志|错误发生前一段时间的OpenHarmony日志|
...
|信号值|信号|解释|常见原因|
|-|-|-|-|
|4|SIGILL|非法指令|PC跳转错误、PAC校验失败、编译平台不兼容|
|5|SIGTRAP|断点或陷阱异常|编译器校验插桩、非void类型返回值的函数未返回|
|6|SIGABRT|进程终止|调用abort()函数终止了进程|
|7|SIGBUS|访问非法内存|内存地址计算错误、mmap文件范围超出范围、物理地址不是类型长度的整数倍|
|8|SIGFPE|浮点数异常|除数为0、浮点树溢出、整数溢出|
|11|SIGSEGV|无效内存访问|空指针、访问了无权限的地址|
|16|SIGSTKFLT|栈溢出|无限递归|
|31|SIGSYS|错误的系统调用|Seccomp拦截，未授权的系统调用使用|

以下是一些经验总结：

- 形如 SIGSEGV(SEGV_MAPERR)@0x00000000 或 参数寄存器的值为0的，应首先检查调用时是否传入了空指针。
形如 SIGSEGV(SEGV_MAPERR)@0x0000000c 或 参数寄存器的值为一个很小的值的，应考虑调用结构体成员是否包含空指针。
- 形如 SIGSEGV(SEGV_MAPERR)@0xffffffffffffffe8 的，无效内存访问，如果使用 llvm-addr2line 后的代码行与STL容器相关，可以考虑多线程STL容器操作问题。STL容器是线程不安全的，如果跨线程共享未加锁，容易出现这类崩溃。
- 形如 SIGABRT 的，一般为用户/框架/C库主动触发崩溃。大部分场景下跳过C库/abort发起的框架库的第一帧即为崩溃原因，如线程创建，文件描述符使用，接口调用时序等。

##### 线程信息

线程信息由四部分组成， 分别是Tid（线程ID）、Name(线程名)、调用栈、寄存器。

崩溃日志中，调用栈信息记录了崩溃时调用栈的状态。调用栈信息的每一行代表一层因崩溃而未返回的调用。调用栈信息的第一行被称为栈顶，最后一行被称为栈底。

定位崩溃时，通常由栈顶开始向栈底逐级检查。

以下是一些经验总结：

- 检查到C/C++标准库（如std、musl等）出错的，通常是传递了错误的参数，应检查栈底方向下一行传递来的参数。
- 检查到napi/jsruntime 等基础库的，通常需要检查ArkTS侧的异常。
- 检查到社区代码的，一般要仔细review这部分社区代码的注释，或在开源社区中搜索是否是已知问题。
多次检查后发现栈顶不规律的、或是反解后的源码位置飘忽不定的，一般是踩内存问题。