...
毕昇编译器是基于LLVM开源软件开发的一款编译器，在无需改动用户代码的条件下能提供比业界主流的开源LLVM或GCC编译器更强大的优化能力，使构建出来的程序运行时长更短、指令数更少，帮助提升应用在设备上的运行流畅度。

当前 DevEco Studio 默认使用 LLVM 的 clang 编译器，从 5.0.3.402 版本开始 DevEco Studio 支持开发者指定使用毕昇编译器进行构建。方法为在应用工程级的 build-profile.json5 中添加如下 nativeCompiler 配置。
...
您可以通过链接到 RNOH 提供的 rnoh_optimization_options intreface 构建目标为您的代码启用一批编译和链接优化选项，参考下方示例用您项目中需要优化的 library 名称替换 rnoh_app

```cmake
target_link_libraries(rnoh_app PUBLIC rnoh_optimization_options)
```

您可以查看[RNOH CMakeList](https://gitcode.com/openharmony-sig/ohos_react_native/blob/0.77.1-rc.1-ohos/packages/tester/harmony/react_native_openharmony/src/main/cpp/CMakeLists.txt) 中 `Bisheng_compile_option` 和 `Bisheng_link_option` 了解具体使用到的优化选项。
...
1. 插桩编译：这一步骤的目的是构建有收集应用运行信息能力的hap。需要在应用顶层 CMakeLists.txt 中临时添加下述构建参数
...
其中 message 一行会在构建日志中打印 C++ 编译器的完整路径，方便您确定 LLVM 工具的位置在后续步骤中使用。`-fprofile-generate` 选项指定将收集到的数据写到盘上哪个文件夹。注意这里设置的 `/data/storage/el2/base/files/pgoprofraw` 是一个应用沙箱内路径，对应文件系统中的实际路径为 `/data/app/el2/100/base/[应用的bundle名， 如 com.example.application]/files/pgoprofraw`
