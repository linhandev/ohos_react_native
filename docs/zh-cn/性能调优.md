...
开启链接时优化（LTO）后链接器可以基于全部编译单元，在更大范围上进行优化，如进行跨文件的函数内联、常量传播等，一般可以提升应用运行速度，减小构建产物的大小。开启LTO需要在应用顶层 CMakeLists.txt 中添加配置，示例如下
...
后续步骤需要使用 lldb 连接到应用发送指令，需要在顶层 build-profile.json5 中临时指定 app.products.buildOption.debuggable 字段为 true 允许 debug。注意这里只是配置应用允许debug，跟打包时选择的 Release/Debug 模式不是同一个概念。可参考下方例子配置
