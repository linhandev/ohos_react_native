...
QDGestureFloatComponentInstance::QDGestureFloatComponentInstance(
    Context context)
  ...
  m_scrollNode.setScrollBarDisplayMode(
      ArkUI_ScrollBarDisplayMode::ARKUI_SCROLL_BAR_DISPLAY_MODE_OFF);
}

facebook::react::Point rnoh::QDGestureFloatComponentInstance::getCurrentOffset()
    const {
...
StackNode& QDGestureFloatComponentInstance::getLocalRootArkUINode() {
  return m_scrollContainerNode;
}

void QDGestureFloatComponentInstance::onPropsChanged(
...
void QDGestureFloatComponentInstance::onChildInserted(
    ComponentInstance::Shared const& childComponentInstance,
    std::size_t index) {
  CppComponentInstance::onChildInserted(childComponentInstance, index);
  m_contentNode.insertChild(
...
void QDGestureFloatComponentInstance::onChildRemoved(
    ComponentInstance::Shared const& childComponentInstance) {
...
void QDGestureFloatComponentInstance::onScrollStop() {
  ...
  if (y <= m_height && y > m_height * m_stopPercentMax) {
    m_scrollNode.scrollTo(0, m_height, true);
      ...
      y < m_height * m_stopPercent ||
      (y <= m_height && y > m_height * m_stopPercent)) {
    m_scrollNode.scrollTo(0, m_height * m_stopPercent, true);
  }
}

void QDGestureFloatComponentInstance::onScrollToCommand(
    float offsetY,
    bool animated) {
