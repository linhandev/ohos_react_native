...
您可以在MyApp的同级创建一个名为`RTNCalculator`的目录，并创建一个`src/specs`子目录，并根据使用的Codegen的版本不同，分别创建`v1`、`v2`子目录，创建后的目录结构是这样的：
```
├── MyApp
└── RTNCalculator
    └── src
...
```TypeScript
// index.ts
import NativeCalculator from "./src/specs/v1/NativeCalculator";
...
1. 创建用于实现模块的`CalculatorModule.ts`
    ...
    如果是CxxTurboModule，不需要在此处实现，空函数体即可。
...
3. 如果是CxxTurboModule，需要一些额外步骤：
   ...
   以CalculatorModule为例，首先需要移除`CalculatorModule.ts`中的函数，仅保留一个空的类
    ...
    然后展开胶水代码文件`RTNCalculator.cpp`中的宏ARK_ASYNC_METHOD_METADATA
